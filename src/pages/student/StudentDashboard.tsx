...
import { 
  ...
  RefreshCcw, AlertCircle, BookOpen 
} from "lucide-react";
...
const initialAtividades = [
  { id: "atv-1", courseId: "1", titulo: "Curso de React Advanced", categoria: "Ensino", horas: 40, data: "12/02/2026", status: "aprovado", feedback: "", arquivoURL: "https://exemplo.com/cert1.pdf" },
  { id: "atv-2", courseId: "1", titulo: "Palestra: Futuro da IA", categoria: "Cultural", horas: 5, data: "10/02/2026", status: "rejeitado", feedback: "Carga horária não visível no certificado.", arquivoURL: "https://exemplo.com/cert2.jpg" },
  { id: "atv-3", courseId: "1", titulo: "Monitoria Algoritmos I", categoria: "Ensino", horas: 60, data: "05/02/2026", status: "pendente", feedback: "", arquivoURL: "https://exemplo.com/cert3.pdf" },
  { id: "atv-4", courseId: "2", titulo: "Workshop Gestão Financeira", categoria: "Extensão", horas: 15, data: "01/03/2026", status: "aprovado", feedback: "", arquivoURL: "https://exemplo.com/cert4.pdf" },
];
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8 bg-[#F8FAFC] min-h-screen">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Acompanhe suas atividades complementares</p>
      </div>

      {/* Seletor de Cursos (Funcional) */}
      <div className="space-y-3">
        <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Meus Cursos</h2>
          ...
          {initialCourses.map((c) => (
            ...
            >
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                activeCourseId === c.id ? "bg-[#0066FF] text-white" : "bg-slate-100 text-slate-400"
              }`}>
                <BookOpen className="h-5 w-5" />
              </div>
              <div>
                <p className={`font-bold text-sm ${activeCourseId === c.id ? 'text-[#0066FF]' : 'text-slate-700'}`}>{c.name}</p>
                <p className="text-xs text-slate-500">{c.aprovadas}/{c.meta}h completadas</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <Card className="border-none shadow-sm bg-white rounded-2xl">
        <CardContent className="p-6 sm:p-8">
          <div className="flex justify-between items-end mb-6">
            <div>
              <h3 className="text-xl font-bold text-slate-900">{activeCourse.name}</h3>
              <p className="text-sm text-slate-500 font-medium text-[12px] uppercase tracking-tight">Meta: {activeCourse.meta} horas</p>
            </div>
          ...
          <div className="h-4 w-full bg-slate-100 rounded-full overflow-hidden">
             <div className="h-full bg-[#0066FF] transition-all duration-700" style={{ width: `${pct}%` }} />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-slate-800">Atividades de {activeCourse.name}</h2>
        <div className="grid grid-cols-1 gap-3">
          {filteredAtividades.map((atv) => (
            <Card key={atv.id} className="border-none shadow-sm overflow-hidden">
              <CardContent className="p-0">
                <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-bold text-slate-900 text-sm sm:text-base truncate">{atv.titulo}</h4>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[10px] font-bold uppercase text-slate-400">{atv.categoria}</span>
                        <span className="text-xs font-semibold text-slate-500">{atv.horas}h</span>
                        <span className="text-xs text-slate-400">{atv.data}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-none pt-3 sm:pt-0">
                    <div className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase ${
                      atv.status === 'aprovado' ? 'text-emerald-600 bg-emerald-50' : 
                      atv.status === 'rejeitado' ? 'text-red-600 bg-red-50' : 'text-orange-600 bg-orange-50'
                    }`}>
                ...
                {atv.status === 'rejeitado' && atv.feedback && (
                  <div className="bg-red-50/50 px-4 py-3 border-t border-red-100 flex items-start gap-2 animate-in slide-in-from-top-1">
                    ...
                    <p className="text-xs text-red-700 italic"><strong>Motivo:</strong> {atv.feedback}</p>
                  </div>
                )}
              </CardContent>
            </Card>
