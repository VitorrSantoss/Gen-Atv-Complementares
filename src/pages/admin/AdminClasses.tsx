...
const AdminClasses = () => {
  ...
  const handleSave = () => {
    ...
    if (isEditing) {
      setClasses(
        classes.map((c) => (c.id === formData.id ? { ...c, ...classData } : c)),
      );
    } else {
      setClasses([
        ...classes,
        { ...classData, id: Math.random().toString(36).substring(2, 9) },
      ]);
    }
    setDialogOpen(false);
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      toast({
        ...
        description: "Turmas cadastradas em lote com sucesso.",
      });
    ...
    <div className="p-8 space-y-6 bg-slate-50 min-h-[calc(100vh-4rem)]">
            ...
            <TableBody>
              {filtered.map((cls) => (
                <TableRow key={cls.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={cls.status === "ativo" ? "default" : "secondary"}
                      className={
                        cls.status === "ativo"
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle className="text-red-600">Excluir Turma</DialogTitle>
          </DialogHeader>
          <div className="py-4 text-slate-600">
            Tem certeza que deseja excluir a turma{" "}
            <strong>{classToDelete?.name}</strong>? Esta ação não removerá os
            alunos associados.
          </div>
