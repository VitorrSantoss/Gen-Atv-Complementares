...
import { UserPlus, Trash2, Pencil, Loader2, ShieldAlert, GraduationCap, UserCog, Filter, ChevronLeft, ChevronRight } from "lucide-react";
...
const AdminUsers = () => {
  ...
  const filteredUsers = users.filter(u => filterProfile === "TODOS" || u.perfil === filterProfile);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: error.response?.data?.message || error.response?.data || "Erro interno.", 
        variant: "destructive" 
      });
    }
  };
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      {/* --- BARRA DE FILTROS --- */}
      <div className="flex items-center gap-3 bg-card p-3 rounded-lg border border-border w-full sm:max-w-xs">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent>
          {/* ... Conteúdo do modal mantido igual (Nome, Email, Senha, Perfil) ... */}
          <DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input placeholder="Ex: Ana Souza" value={formData.nome} onChange={(e) => setFormData({...formData, nome: e.target.value})} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">E-mail</label>
              <Input type="email" placeholder="usuario@senac.com" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Senha {editingId && "(Opcional)"}</label>
              <Input type="password" placeholder={editingId ? "Manter senha atual" : "********"} value={formData.senha} onChange={(e) => setFormData({...formData, senha: e.target.value})} />
            </div>
            <div className="space-y-2">
              ...
              {filteredUsers.length > 0 && (
                <div className="flex items-center justify-between border-t border-border pt-4 mt-4">
                  <div className="text-sm text-muted-foreground">
                    Mostrando <span className="font-medium text-foreground">{startIndex + 1}</span> a <span className="font-medium text-foreground">{Math.min(startIndex + itemsPerPage, filteredUsers.length)}</span> de <span className="font-medium text-foreground">{filteredUsers.length}</span> resultados
                  </div>
